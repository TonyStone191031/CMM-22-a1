...
public:
    ...
    double evaluate(const VectorXd& x) const override {
        ...
        e = 0.5 * (endEffectorPosition(*linkage, x) - *target).transpose() * (endEffectorPosition(*linkage, x) - *target);
    ...
    VectorXd gradient(const VectorXd& x) const override {
        ...
        // put your code in this function
        VectorXd grad = dendEffector_dangles(*linkage, x).transpose() * (endEffectorPosition(*linkage, x) - *target);
    ...
    Matrix2d hessian(const VectorXd &x) const {
        ...
        Vector2d error = endEffectorPosition(*linkage, x) - *target;

        for (int i = 0; i < 2; ++i) {
            for (int j = 0; j < 2; ++j) {
                hess(i, j) = tensor[i](0, j) * error[0] + tensor[i](1, j) * error[1];
            }
        }
