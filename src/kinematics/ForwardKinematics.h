...
std::array<Vector2d, 3> forwardKinematics(const Linkage &linkage, const Vector2d &angles) {
    ...
    Vector2d p1 = p0 + linkage.length[0] * Vector2d(cos(angles[0]), sin(angles[0]));
    Vector2d p2 = p1 + linkage.length[1] * Vector2d(cos(angles[0]+angles[1]), sin(angles[0]+angles[1]));
    return {p0, p1, p2};
}
